...
if [ "$OSTYPE" == 'msys' ]; then
	...
	if [ ! -z "$SEVENZIP_LOCATION" ]; then
		...
		cd ~/bin
		download_file http://dl.dropbox.com/u/5581063/7za.exe $(pwd)/7za.exe e92604e043f51c604b6d1ac3bcd3a202
	fi
	SEVENZIP_LOCATION=`which 7za`
	if [ -z "$SEVENZIP_LOCATION" ]; then
	...
	if [ ! -z "$JOM_LOCATION" ]; then
		...
		cd ~/bin
		download_file http://dl.dropbox.com/u/5581063/jom.exe $(pwd)jom.exe 335428f223d36f0a39faaa845222346d
	fi
	JOM_LOCATION=`which jom`
	if [ -z "$JOM_LOCATION" ]; then
