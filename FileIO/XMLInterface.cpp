	...
	while (!cond.isNull())
	{
		...
		for (int i = 0; i < condProperties.count(); i++)
		{
			if (condProperties.at(i).nodeName().compare("ProcessType") == 0)
				c->setProcessType(convertProcessType(condProperties.at(i).toElement().text().toStdString()));
			if (condProperties.at(i).nodeName().compare("PrimaryVariable") == 0)
				c->setProcessPrimaryVariable(convertPrimaryVariable(condProperties.at(i).toElement().text().toStdString()));
