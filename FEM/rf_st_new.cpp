...
void CSourceTerm::ReadDistributionType(std::ifstream* st_file)
{
	...
	if (dis_type_name.compare (convertDisTypeToString (this->getProcessDistributionType())) !=
	    0)
	{
		std::cerr << "Error in CSourceTerm::ReadDistributionType (): dist_type_name #" <<
		dis_type_name << "#, new: " << convertDisTypeToString (
		        this->getProcessDistributionType()) << std::endl;
		exit (1);
	}
	...
	if (this->getProcessDistributionType() == FiniteElement::ANALYTICAL)
	{
		...
		analytical = true;
		analytical_processes.push_back(convertPrimaryVariableToString (
	...
	while (!st_file.eof())
	{
		...
		if (line_string.find("#STOP") != std::string::npos)
		{
			...
			for (size_t i = 0; i < no_source_terms; i++)
				...
				{
					no_an_sol++;
					number_of_terms = std::max(
					        st_vector[i]->getNumberOfTerms(), number_of_terms);
				}
			if (no_an_sol > 0)
				for (size_t i = 0; i < no_source_terms; i++)
				{
...
void ST2RHS(std::string pcs_function, double* rhs_vector)
{
	...
	{
		m_st = st_vector[j];
		if (pcs_function.compare(convertPrimaryVariableToString (m_st->
		                                                         getProcessPrimaryVariable()))
		    == 0)
		{
                           ...
                           std::string this_pv_name)
{
	...
	if (m_msh)                            //WW
	{
		...
		for (long i = 0; i < no_st; i++)
		{
			...
			if ((convertProcessTypeToString (source_term->getProcessType ()).compare(
			             pcs_type_name) == 0)
			    && (convertPrimaryVariableToString(source_term->
			                                       getProcessPrimaryVariable()).compare(
			                pcs_pv_name) == 0))
				...
				else
				{
					for (k = 0; k < 3; k++) // Three nodes
						...
						for (l = 0; l < 3; l++) // Gauss points
						{
							...
							NVal[G2L[e_nodes[k]->GetIndex()]] += 0.5 *
							                                     (v1 +
							                                      v2
							                                      +
							                                      eta *
                               ...
                               CNodeValue* cnodev)
{
	...
	if (h_this_shifted + epsilon > h_cond)
		relPerm_epsilon = GetRelativeCouplingPermeability(m_pcs_this,
		                                                  h_this
		                                                  + epsilon,
		                                                  m_st,
		                                                  cnodev->msh_node_number);
		...
		relPerm_epsilon = GetRelativeCouplingPermeability(
		        ...
		        m_st,
		        cnodev->
		        msh_node_number_conditional);  // richards or groundwater
...
**************************************************************************/
double GetRelativeCouplingPermeability(const CRFProcess* pcs,
                                       double head,
                                       ...
                                       long msh_node)
{
	...
	{
		std::cout << "!!!!! Coupling flux upwinding not implemented for this case !!!!!" <<
		std::endl;
		relPerm = 1;
	}
...
//      double z_cond = m_pcs_cond->m_msh->nod_vector[cnodev->msh_node_number_conditional]->Z();
		double const* const xyz_cond (
		        m_pcs_cond->m_msh->nod_vector[cnodev->msh_node_number_conditional]->getData());
...
void GetCriticalDepthNODValue(double &value, CSourceTerm* m_st, long msh_node)
{
	...
	else
	{
		...
		{
			msh_ele
			        = m_pcs_this->m_msh->nod_vector[msh_node]->getConnectedElementIDs()
			          [0];
...
void GetNormalDepthNODValue(double &value, CSourceTerm* st, long msh_node)
{
	...
	if (st->getNormalDepthSlope() == -1)  // TF: WARNING comparison of double via ==
	{
		...
		double elementlength =
		        sqrt(MathLib::sqrDist(m_ele->GetNode(1)->getData(),
		                              m_ele->GetNode(0)->getData()));
...
//                + MathLib::fastpow(m_ele->GetNode(1)->Z() - m_ele->GetNode(0)->Z(), 2));
		S_0 =
		        (m_ele->GetNode(1)->getData()[2] -
		         m_ele->GetNode(0)->getData()[2]) / elementlength;
		if (S_0 < 0)
			S_0 = -S_0;
	}
	else
	...
	if (mmp_vector[group]->channel == 1)
	{
		...
		value_for_jacobi = pow(flowdepth_epsilon * width / (2
		                                                    * flowdepth_epsilon + width),
		                       depth_exp) * flowdepth_epsilon
		                   * temp + value;
	}
	else
	{
...
void CSourceTermGroup::SetPLY(CSourceTerm* st, int ShiftInNodeVector)
{
	...
	if (old_ply)
	{
		...
		m_msh->GetNODOnPLY(
		        static_cast<const GEOLIB::Polyline*>(st->getGeoObj()), ply_nod_vector);
		m_msh->setMinEdgeLength (min_edge_length);

		if (st->isCoupled())
			SetPolylineNodeVectorConditional(st, ply_nod_vector, ply_nod_vector_cond);

		SetPolylineNodeValueVector(st,
		                           ply_nod_vector,
		                           ply_nod_vector_cond,
		                           ply_nod_val_vector);

		st->SetNodeValues(ply_nod_vector,
		                  ply_nod_vector_cond,
		                  ply_nod_val_vector,
		                  ShiftInNodeVector);
	} // end polyline
	...
	if (st->node_averaging)
	{
		if (m_msh_cond)
		{
			if (pcs_type_name == "RICHARDS_FLOW")
			{
				...
				m_msh_cond->GetNODOnPLY(static_cast<const GEOLIB::Polyline*>(st->
				                                                             getGeoObj(
				                                                                     )),
				                        ply_nod_vector_cond);
                                                  ...
                                                  std::vector<double>& ply_nod_val_vector) const
{
	...
	if (distype == FiniteElement::LINEAR || distype == FiniteElement::LINEAR_NEUMANN)
	{
		...
		if (polyline)
		{
			...
			m_msh->getPointsForInterpolationAlongPolyline (polyline,
			                                               nodes_as_interpol_points);
			...
			for (long k = 0; k < (long) st->DistribedBC.size(); k++)
			{
				for (long l = 0; l < (long) m_ply->point_vector.size(); l++)
					    ...
					    == m_ply->point_vector[l]->id)
					{
						...
						m_ply->point_vector[l]->setPropert (
						        st->DistribedBC[k]);
						break;
					} // end l
				// end k
                                ...
                                const std::vector<double>&node_values, int ShiftInNodeVector)
{
	...
	for (size_t i = 0; i < number_of_nodes; i++)
	{
		...
		if (_coupled)             // JOD 4.7.10
		{
			...
			// JOD 4.10.01
			if ((getProcessType() == OVERLAND_FLOW || getProcessType() ==
			     GROUNDWATER_FLOW)
			    && node_averaging)
			{
				double weights = 0;
...
std::string CSourceTerm::DirectAssign_Precipitation(double current_time)
{
	...
	else
	{
		...
		for(i = 0; i < size; i++)
		{
			...
			else if((tim_val - current_time) < step_f)
			{
...
double CSourceTerm::GetAnalyticalSolution(long location)
{
	...
	t0 = tn = tnn = source = gradient = val1 = val2 = node_area = flux_area
	                                                                      = 0.0;
	idx =
	        m_pcs->GetNodeValueIndex(convertPrimaryVariableToString(this->
	                                                                getProcessPrimaryVariable()));
...
void CSourceTerm::SetNodePastValue(long n, int idx, int pos, double value)
{
	...
	CRFProcess* m_pcs = NULL;
	m_pcs =
	        PCSGet(convertProcessTypeToString(getProcessType()),
...
**************************************************************************/
void GetCouplingFieldVariables(double* h_this,
                               double* h_cond,
                               double* h_shifted,
                               double* h_averaged,
                               double* z_this,
                               ...
                               CNodeValue* cnodev)
{
	...
	*z_cond
	...
	nidx =
	        m_pcs_this->GetNodeValueIndex (convertPrimaryVariableToString (m_st->
	                                                                       getProcessPrimaryVariable()))
	        + 1;
				...
				*h_averaged
				        += cnodev->msh_node_weights_averaging[i]
				           * (m_pcs_this->GetNodeValue(
				                      ...
				                      nidx)
				              - m_pcs_this->m_msh->nod_vector[cnodev->
				                                              msh_node_numbers_averaging
				                                              [i]]->getData()[2]);
