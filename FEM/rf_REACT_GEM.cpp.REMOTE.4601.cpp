	...
	if ( Load_Init_File ( Project_path ) )
	{
		...
		for ( ii = 0; ii < nNodes; ii++ )
		{
			...
			if ( !( m_NodeStatusCH[i] == OK_GEM_AIA || m_NodeStatusCH[i] == OK_GEM_SIA ) )
			{
				...
				{
					cout <<
					"critical error: Initial GEMs run after Read GEMS failed at node "
					     << i << endl;
					...
					cout <<
					"error: Initial GEMs run after Read GEMS gives bad result..proceed in any case "
					     << i << endl;
...
short REACT_GEM::Init_RUN()
{
	...
	for ( ii = 0; ii < (int) m_kin.size(); ii++ )
	{
		...
		for ( j = 0; j < nPH; j++ )
		{
			...
			" component start number " <<  m_kin[ii].dc_counter << endl;
		...
		if ( m_kin[ii].phase_number < 0 || m_kin[ii].phase_number >= nPH )
		{
			...
			" in phase number " << m_kin[ii].phase_number << endl;
	}
	...
	for ( i = 0; i < nNodes; i++ )
	{
		...
		if ( this_pcs )
		{
			...
			if ( IsThisPointBCIfYesStoreValue ( i, this_pcs, BCValue ) )
			{
				...
				cout << "Node " << i << ", Comp " <<
				this_pcs->pcs_primary_function_name[0] << " ,Value " << BCValue <<
				" is boundary node" << endl;
				m_boundary[i] = 1;
			}
			else
	...
	for ( ii = 0; ii < nNodes; ii++ )
	{
		...
		if ( !( m_NodeStatusCH[in] == OK_GEM_AIA || m_NodeStatusCH[in] == OK_GEM_SIA ) )
		{
			...
			if ( !( m_NodeStatusCH[in] == OK_GEM_AIA || m_NodeStatusCH[in] ==
			        OK_GEM_SIA ) )
			{
				cout << " Error: Init Loop failed when running GEM on Node #" <<
				in << "." << endl;
		...
		if ( m_flow_pcs->GetRestartFlag() < 2 )
			REACT_GEM::CalcPorosity ( in );            //during init it should be always done, except for restart !!!
...
short REACT_GEM::GetInitialReactInfoFromMassTransport ( int timelevel )
{
	...
	for ( long node_i = 0; node_i < nNodes; node_i++ )
	{
		...
		if ( flag_transport_b == 0 )
			REACT_GEM::GetDCValue_MT ( node_i,
			                           timelevel,
			                           m_xDC + node_i * nDC,
			                           m_xDC_pts + node_i * nDC,
			                           m_xDC_MT_delta + node_i * nDC );
		else if ( ( flag_transport_b == 1 ) && ( m_flow_pcs->GetRestartFlag() < 2 ) )
...
short REACT_GEM::Run_MainLoop ( )
{
	...
	if ( m_ShuffleGems )
		ShuffleIterations ( m_IterDoneIndex, nNodes );
	for ( ii = 0; ii < nNodes; ii++ )
	{
		in = m_IterDoneIndex[ii]; // m_IterDoneIndex can be sortet to account for nodes that spend a lot of time for gems
		// cout <<"iteration "<< ii << " node " << in <<endl;
		...
		else // calculate if not on a boundary or transport causes differences in solutes
		{
			...
			if ( !( m_NodeStatusCH[in] == OK_GEM_AIA || m_NodeStatusCH[in] ==
			        ...
			        m_NodeStatusCH[in] ==
			        BAD_GEM_SIA) )                                                                                                               // ups...failed..try again without SIMPLEX
			{
...
// seems not to work...better copy old values!!!    REACT_GEM::MassToConcentration ( in, 1 /* node failed */); //This is for failed nodes!!!!!
	...
	{
		if ( myrank == 0 )
			cout << "GEMS: Run_Main resorting. timestep " << aktueller_zeitschritt <<
			endl;
		...
		case 1:                   // for "GROUNDWATER_FLOW";
			if ( aktueller_zeitschritt > 0 ) // not for first time step
			{
				...
				else
				{
					...
					pressure = m_flow_pcs->GetNodeValue ( node_Index,
					                                      indx + timelevel );
...
double REACT_GEM::GetComponentValue_MT ( long node_Index, string m_component, int timelevel )
{
	...
	for ( size_t i = 0; i < pcs_vector.size(); i++ )
	{
		...
		//                if ( m_pcs->pcs_type_name.compare ( "MASS_TRANSPORT" ) == 0 ) {
		if ( m_pcs->getProcessType() == MASS_TRANSPORT)
			if ( strcmp ( m_pcs->pcs_primary_function_name[0],
			              m_component.c_str() ) == 0 )
				m_comp_value = m_pcs->GetNodeValue ( node_Index,
		...
		if ( m_pcs->getProcessType() == MASS_TRANSPORT)
		{
			...
			if ( x_Component > -1 )
				*( m_Phase + x_Component ) = m_pcs->GetNodeValue (
				        node_Index,
				        m_pcs->
				        GetNodeValueIndex ( str ) + timelevel );
			else
			{
...
void REACT_GEM::MassToConcentration ( long in /*idx of node*/,int i_failed )    //attention second argument is not timestep. I is a flag that indicates if we deal with failed nodes!!!!...do not get data from GEMS for this nodes
{
	...
	if ( ( fluid_volume <= 0.0 ) )
	{
		cout << "fluid volume negative or zero" << fluid_volume  << " node " << in <<
		" " << m_fluid_volume[in] << endl;
#ifdef USE_MPI_GEMS
		MPI_Finalize();           //make sure MPI exits
#endif
		exit ( 1 );
	...
	case 3:                               // Richards flow
		...
		m_excess_water[in] = m_fluid_volume[in] - m_porosity[in] * m_pcs->GetNodeValue (
		        in,
		        idx
		        + 1 );
		...
		skal_faktor =
		        ( m_porosity[in] *
		          m_pcs->GetNodeValue ( in,idx + 1 ) ) / m_fluid_volume[in];      //mol amount of water in first phase
	...
	if ( flag_transport_b == 0 )
		for ( j = 0; j < nDC; j++ )
		{
			...
			if ( ( dCH->ccDC[j] == 'S' ) || ( dCH->ccDC[j] == 'E' ) ||
			     ( dCH->ccDC[j] == 'T' ) || ( dCH->ccDC[j] == 'L' ) )
	...
	// here we adjust the b vector
		...
		for ( j = 0; j < nIC; j++ )
		{
			...
			if ( idx_hydrogen == j )
				m_soluteB[i] = m_bPS[j] - ( 2.0 * m_xDC[in * nDC + idx_water] );
			else if ( idx_oxygen == j )
				m_soluteB[i] = m_bPS[j] - m_xDC[in * nDC + idx_water];
			else
				m_soluteB[i] = m_bPS[j];
	...
	for ( ii = 0; ii < ( int ) m_kin.size(); ii++ )
	{
		...
		// kinetic_mocel==3 only precipitation (no dissolution)
		{
			...
			for ( i = 0; i < m_kin[ii].n_activities; i++ )
			{
				...
				if ( idx < 0 )
				{
					cout << "CalcReactionRate: no DC-name " <<
					m_kin[ii].active_species[i] << " found" << endl;
#ifdef USE_MPI_GEMS
					MPI_Finalize(); //make sure MPI exits
#endif
			...
			{
				cout << "dmdt " << dmdt << " is NaN " << " sa " << sa << " rra " <<
				rra << " rrn " << rrn << " rrb " << rrb << endl;
...
void REACT_GEM::GetGEMResult_MPI ( void )
{
	...
	MPI_Allreduce ( m_fluid_volume_buff,
	                ...
	                MPI_SUM,
	                MPI_COMM_WORLD );
	MPI_Allreduce ( m_gas_volume_buff,
	                m_gas_volume,
	                nNodes,
	                MPI_DOUBLE,
	                MPI_SUM,
	...
	MPI_Allreduce ( m_excess_water_buff,
	                ...
	                MPI_SUM,
	                MPI_COMM_WORLD );
	MPI_Allreduce ( m_excess_gas_buff,
	                m_excess_gas,
	                nNodes,
	                MPI_DOUBLE,
	                MPI_SUM,
